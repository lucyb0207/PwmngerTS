...
import { Mail, Lock, Sparkles, ArrowRight, Shield, Loader2 } from "lucide-react";
...
export const RegisterVault: React.FC<RegisterVaultProps> = ({
  ...
  return (
    <div className="card-premium animate-fade-in" style={{ padding: "40px", borderRadius: "var(--radius-xl)", position: "relative", overflow: "hidden" }}>
      {loading && (
        <div className="loading-overlay animate-fade-in" style={{
          ...
          <div style={{ textAlign: "center" }}>
             <h3 style={{ margin: 0, fontSize: "18px", color: "#fff" }}>Constructing Secure Vault</h3>
             <p style={{ margin: "4px 0 0", fontSize: "12px", color: "var(--text-dim)" }}>Deriving encryption keys with Argon2id...</p>
          </div>
        </div>
      )}

      <div style={{ textAlign: "center", marginBottom: 32 }}>
        <h2 style={{ margin: "0 0 8px", fontSize: "24px", fontWeight: 700, letterSpacing: "-0.025em" }}>{isWizard ? "Secure Your Account" : "Create Vault"}</h2>
      ...
      <div style={{ marginTop: 32 }}>
        ...
        >
          {loading ? "Constructing Vault..." : isWizard ? "Initialize My Vault" : "Create Secure Vault"} 
          {!loading && (isWizard ? <ArrowRight size={16} /> : <Sparkles size={16} />)}
        </Button>
      </div>
